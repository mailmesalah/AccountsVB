...
Dim rs As Recordset
    ...
    Set rs = db.OpenRecordset("Select AccountRegister.* From AccountRegister Where (AccountRegister.TransactionNo = '" & Trim(TVoucherNo.Text) & "' ) And (AccountRegister.Type = 'P' )")
...
Dim rs As Recordset, sTransactionNo As String
    ...
    Set rs = db.OpenRecordset("Select Max(Val(AccountRegister.TransactionNo)) As TNo From AccountRegister Where (AccountRegister.Type = 'P' )")
    If rs.RecordCount > 0 Then
...
Dim r As Long
    ...
    Set rs = db.OpenRecordset("Select AccountRegister.*,AccountMaster.AccountName From AccountRegister,AccountMaster Where (AccountRegister.TransactionNo = '" & Trim(sTransactionNo) & "' ) And (AccountRegister.Type = 'P' ) And (AccountMaster.Code=AccountRegister.AccountCode)")
    If rs.RecordCount > 0 Then
