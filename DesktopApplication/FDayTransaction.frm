...
Dim gVoucherNo As Single, gAccount As Single, gNarration As Single, gReceipt As Single, gPayment As Single, gAccountCode As Single
...
Dim rs As Recordset, lTransactionNo As String, lRecordCount As Long, r As Long, lBigestNo As Long
    ...
    lBigestNo = 0
    lTransactionNo = getNewTransactionForAccount() - 1
...
Dim rs As Recordset, lYN As Long, bFound As Boolean
    ...
    If (MsgBox("Do you want to Delete this day's Transaction ?", vbDefaultButton2 Or vbYesNo) = vbYes) Then
        Set rs = db.OpenRecordset("Select AccountRegister.* From AccountRegister Where (AccountRegister.TransactionDate = cDate('" & DTPDate.Value & "'))")
        While rs.EOF = False
            bFound = True
            rs.Delete
            rs.MoveNext
...
Dim lRowCount As Long, lColCount As Long
    ...
    oExcelSheet.Cells(1, 1).Value = "Day Transaction of " & Format(DTPDate.Value, "dd-MM-yyyy")
...
On Error Resume Next
    ...
    Kill App.Path & "\Reports\DayTransaction " & Format(DTPDate.Value, "dd-MMM-yyyy") & ".xlsx"

  ' Save the file as a native XLS file...
    oExcel.SaveAs App.Path & "\Reports\DayTransaction " & Format(DTPDate.Value, "dd-MMM-yyyy") & ".xlsx"
  ...
  ' Close the OLE object and remove it...
    ...
    xlTmp.Visible = True
    xlTmp.Workbooks.Open App.Path & "\Reports\DayTransaction " & Format(DTPDate.Value, "dd-MMM-yyyy") & ".xlsx"
...
Dim rs As Recordset, r As Long
    ...
    Set rs = db.OpenRecordset("Select AccountRegister.*,AccountMaster.AccountName From AccountRegister,AccountMaster Where (AccountRegister.TransactionDate = cDate('" & DTPDate.Value & "') And AccountMaster.Code=AccountRegister.AccountCode ) Order By Val(AccountRegister.TransactionNo)")
    MGrid.Rows = 0
    If rs.RecordCount > 0 Then
